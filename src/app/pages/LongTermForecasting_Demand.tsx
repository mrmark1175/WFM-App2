...
const BLEND_PRESETS: BlendPreset[] = [
  ...
  { id: "dedicated", label: "Dedicated per channel", description: "No channel blending", pools: [["voice"], ["email"], ["chat"], ["cases"]] },
];
...
function buildBlendConfiguration(selectedChannels: Record<ChannelKey, boolean>, poolingMode: PoolingMode): BlendConfiguration {
  ...
  if (poolingMode === "dedicated") {
    return {
      ...
      description: "Selected channels stay separate",
...
function buildIndexedLongTermHistory(rows: LongTermActualRecord[]): number[] {
  ...
  const isOneBasedMonth = rows.some((row) => Number(row.month_index) === 12);
  const indexed = new Map<number, number>();
  let maxIndex = -1;
  for (const row of rows) {
    const yearIndex = Number(row.year_index);
    const rawMonthIndex = Number(row.month_index);
    if (!Number.isInteger(yearIndex) || !Number.isInteger(rawMonthIndex)) continue;
    const monthIndex = isOneBasedMonth ? rawMonthIndex - 1 : rawMonthIndex;
    if (monthIndex < 0 || monthIndex > 11) continue;
    const absoluteIndex = yearIndex * 12 + monthIndex;
    indexed.set(absoluteIndex, Number(row.volume) || 0);
    maxIndex = Math.max(maxIndex, absoluteIndex);
  }
  if (maxIndex < 0) return rows.map((row) => Number(row.volume) || 0);
  return Array.from({ length: maxIndex + 1 }, (_, index) => indexed.get(index) ?? 0);
}
...
const getYearOneForecastBaseline = (
  forecastByChannel: Record<ChannelKey, number[]>,
...
export default function LongTermForecastingDemand() {
          ...
          try {
            ...
            if (actualsResponse.ok) {
              ...
              if (Array.isArray(actuals) && actuals.length > 0) {
                return { channel, data: buildIndexedLongTermHistory(actuals) };
              }
            }
          } catch (error) {
  ...
  const allForecastMonths = useMemo(() => {
    ...
    return Array.from({ length: effectivePlanningMonths }, (_, i) => {
      ...
      const year1Forecast = getYearOneForecastBaseline(forecastVolumesByChannel, effectiveFinalHistoricalDataByChannel, detailChannel);
  ...
  const scenarioDemandSummary = useMemo(() => {
    ...
    return scenarioEntries.map((scenario) => {
      ...
      const volumes = months.map((month) => month.totalVolume);
      const peakVolume = volumes.length > 0 ? Math.max(...volumes) : 0;
      const peakMonth = months.find((month) => month.totalVolume === peakVolume)?.month ?? null;
...
Below is live demand forecast data. Write a concise executive-level insight narrative about DEMAND VOLUME ONLY. Keep the response strictly about volume, actuals, variance, re-cut forecast, trend, and seasonality.
...
Rules: cite specific months and numbers; no filler phrases; avoid capacity or headcount language.`;
  ...
  useEffect(() => {
    ...
    setPageData({
      ...
      allModelForecasts,
      scenarioSummary: scenarioDemandSummary,
      kpis,
      assumptions: {
        aht: assumptions.aht,
  ...
  }, [forecastYear, forecastMethod, detailChannel, activeScenario, historicalRowsByYear, allForecastMonths, kpis, assumptions, setPageData, effectiveFinalHistoricalDataByChannel, hwParams, arimaParams, decompParams, scenarioDemandSummary]);
        ...
        <div className="flex flex-col gap-8 pb-12">
          ...
          <div className="flex items-center gap-1 flex-wrap">
            {([
              ...
              { label: "Demand Output",    id: "section-demand-output" },
            ] as const).map((step, i, arr) => (
              <React.Fragment key={step.id}>
                <button
                  type="button"
                  onClick={() => document.getElementById(step.id)?.scrollIntoView({ behavior: "smooth", block: "start" })}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border border-slate-200 bg-white hover:bg-slate-50 hover:border-slate-400 text-slate-600 hover:text-slate-900 transition-colors shadow-sm"
                >
                  <span className="text-slate-400 font-mono">{i + 1}</span>
                  {step.label}
                </button>
                {i < arr.length - 1 && <ChevronRight className="size-3 text-slate-300 shrink-0" />}
              </React.Fragment>
            ))}
          </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-white px-6 py-4 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {(() => {
                const trendChip = (pct: number | undefined) => {
          ...
          <Card id="section-historical" className="order-1 min-w-0 border border-primary/15 shadow-md overflow-hidden">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-foreground/60">Historical volumes feed the forecast engine. Enter actuals for completed months to generate a re-cut.</p>
                  </div>
                  {/* Channel tabs */}
                ...
                <div className="flex flex-wrap gap-3 items-end">
                  {/* Data source toggle */}
                  <div>
                    <span className="text-[11px] font-black uppercase tracking-widest text-foreground/60 block mb-1.5">Data Source</span>
                    <div className="flex rounded-lg border border-border p-0.5 bg-muted/40">
                      <button type="button" onClick={() => setDataSourceMode("api")}
                      ...
                      <button type="button" onClick={() => setDataSourceMode("manual")}
                  ...
                  {dataSourceMode === "api" && (
                    <div className="flex items-center gap-2 ml-auto flex-wrap">
                      <Button variant="outline" size="sm" className="gap-2 h-9" onClick={handleClearApiData}>
                        <Trash2 className="size-3.5" />Clear API Data
                      </Button>
                      <Button variant="outline" size="sm" className="gap-2 h-9" onClick={handleRestoreApiData} disabled={!canRestoreApiData}>
                        <RotateCcw className="size-3.5" />Restore API Data
                      </Button>
                      <Button variant="outline" size="sm" className="gap-2 h-9" onClick={handleResetAllOverrides} disabled={overrideCount === 0}>
                        <RotateCcw className="size-3.5" />Reset Overrides
                      </Button>
                      <Button size="sm" className="gap-2 h-9 bg-violet-600 hover:bg-violet-700 text-white border-0"
                        onClick={runOutlierAnalysis} disabled={isAnalyzing || historicalSourceRows.length < 4}>
                  ...
                  {dataSourceMode === "manual" && (
                    <div className="flex items-center gap-3 ml-auto">
                      ...
                      <span className="text-xs text-muted-foreground">Checks entered values for anomalies</span>
                    </div>
                  )}
                </div>
                  ...
                  <div className="rounded-xl border border-violet-200/70 dark:border-violet-800/40 bg-violet-50/60 dark:bg-violet-950/20 overflow-hidden">
                    <button type="button"
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <span className="text-sm font-bold text-violet-900 dark:text-violet-200">AI Outlier Analysis</span>
                              ...
                              {outlierResults.map((result) => (
                                <div key={result.index}
                                  className={`rounded-lg border px-4 py-3 flex flex-col sm:flex-row sm:items-start gap-3 ${
                                    result.applied ? "border-emerald-200 bg-emerald-50/60 dark:bg-emerald-950/20 dark:border-emerald-800/40"
                                  ...
                                  <div className="flex items-center gap-2 shrink-0">
                                    ...
                                    <Badge variant="outline" className="text-[10px] border-violet-300 text-violet-700">Z {result.modZScore}</Badge>
                                  </div>
                  ...
                  return (
                    <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 flex-wrap">
                      ...
                      <span className="text-xs font-semibold text-amber-800">
                        {missing} completed {missing === 1 ? "month has" : "months have"} no actuals entered. Add them in the Actual column to unlock the Re-cut factor.
                      </span>
                    </div>
                  );
                })()}
            ...
            <CardContent className="p-0 overflow-x-auto">
              {(() => {
                ...
                const hist1 = historicalRowsByYear.find((g) => g.year === String(yr1))?.rows ?? [];
                const hist2 = historicalRowsByYear.find((g) => g.year === String(yr2))?.rows ?? [];
                // build monthIndex→row maps for quick lookup
                ...
                return (<>
                  <Table className="table-fixed min-w-[820px]">
                    <colgroup>
                      <col style={{ width: "10%" }} />
                      <col style={{ width: dataSourceMode === "api" ? "14%" : "12%" }} />
                      <col style={{ width: dataSourceMode === "api" ? "14%" : "12%" }} />
                      <col style={{ width: "13%" }} />
                      <col style={{ width: "16%" }} />
                      <col style={{ width: "10%" }} />
                      {activeRecutFactor != null && <col style={{ width: "13%" }} />}
                    </colgroup>
                    <TableHeader className="bg-muted/50">
                      <TableRow className="hover:bg-transparent">
                        ...
                        <TableHead className="px-3 text-center text-[10px] font-black uppercase tracking-widest text-foreground/70 whitespace-nowrap">{yr1} Volume</TableHead>
                        <TableHead className="px-3 text-center text-[10px] font-black uppercase tracking-widest text-foreground/70 whitespace-nowrap">{yr2} Volume</TableHead>
                        <TableHead className="px-3 text-center text-[10px] font-black uppercase tracking-widest text-primary whitespace-nowrap">{forecastYear} Forecast</TableHead>
                    ...
                    <TableBody>
                      {MONTH_NAMES.map((monthName, monthIdx) => {
                        const h1 = hist1ByMonthIdx.get(monthIdx);
                        const h2 = hist2ByMonthIdx.get(monthIdx);
                        const fc = allForecastMonths[monthIdx];
                        if (!fc) return null;
                        const isSaving = savingActuals.has(fc.actualKey);
                        const isSaved = savedActuals.has(fc.actualKey);
                        const outlier1 = outlierResults?.find((r) => r.index === (h1?.index ?? -1));
                        const outlier2 = outlierResults?.find((r) => r.index === (h2?.index ?? -1));

                        const renderHistCell = (
                          histRow: typeof h1,
                          startIdx: number,
                          outlier: typeof outlier1,
                        ) => {
                            ...
                            return (
                              <TableCell className="px-2 py-1.5 text-center align-middle">
                                <Input
                                  inputMode="numeric"
                                  pattern="[0-9]*"
                                  value={visibleHistoricalOverrides[startIdx + monthIdx] ?? ""}
                                  onChange={(e) => handleOverrideChange(startIdx + monthIdx, e.target.value)}
                                  onBlur={() => handleOverrideBlur(startIdx + monthIdx)}
                                  onPaste={(e) => handleManualGridPaste(startIdx, monthIdx, e)}
                                  placeholder="—"
                          ...
                          // API mode
                          if (!histRow) {
                            return <TableCell className="px-2 py-1.5 text-center text-xs text-muted-foreground align-middle">—</TableCell>;
                          }
                          return (
                            <TableCell className={`px-2 py-1.5 text-center align-middle ${
                              outlier && !outlier.applied
                                ? outlier.severity === "extreme" ? "bg-rose-50/40 dark:bg-rose-950/10" : "bg-amber-50/40 dark:bg-amber-950/10"
                                : histRow.canEdit ? "bg-amber-50/60 dark:bg-amber-950/10" : ""
                            }`}>
                              {histRow.canEdit ? (
                                <div className="flex items-center gap-1 justify-center">
                                  <Input
                                    inputMode="numeric"
                                    pattern="[0-9]*"
                                    value={histRow.overrideVolume}
                                    onChange={(e) => handleOverrideChange(histRow.index, e.target.value)}
                                    onBlur={() => handleOverrideBlur(histRow.index)}
                                    placeholder={String(histRow.apiVolume)}
                                    className="h-7 w-24 text-center font-mono tabular-nums text-xs px-1"
                                  />
                                  <Button variant="ghost" size="sm" className="h-6 px-1 text-[10px]"
                                    onClick={() => handleResetMonthOverride(histRow.index)}>✕</Button>
                                </div>
                              ) : (
                                <div className="flex items-center justify-center gap-1">
                                  <span className="font-mono text-xs tabular-nums">{formatInteger(histRow.finalVolume)}</span>
                                  {outlier && !outlier.applied && (
                                    <UITooltip>
                                      <TooltipTrigger asChild>
                                        <AlertTriangle className={`size-3 cursor-help shrink-0 ${outlier.severity === "extreme" ? "text-rose-500" : "text-amber-500"}`} />
                                      </TooltipTrigger>
                                      <TooltipContent className="max-w-[260px]">
                                        <p className="text-xs font-semibold mb-1">{outlier.severity === "extreme" ? "Extreme" : "Mild"} Outlier — {outlier.direction === "high" ? "↑ High" : "↓ Low"}</p>
                        ...
                        return (
                          <TableRow key={`${monthName}-${detailChannel}`}
                            className={`hover:bg-muted/30 ${fc.isCompleted ? "bg-blue-50/30 dark:bg-blue-950/10" : ""}`}>
                            {/* Month */}
                            <TableCell className="px-3 py-2 text-center align-middle">
                              <div className="flex items-center justify-center gap-1.5 flex-wrap">
                                <span className="font-bold text-sm">{monthName}</span>
                                {fc.isCompleted && (
                                  <Badge variant="outline" className="text-[10px] h-4 px-1 text-blue-600 border-blue-300">Done</Badge>
                                )}
                              </div>
                            </TableCell>
                            {/* yr1 historical */}
                            {renderHistCell(h1, 0, outlier1)}
                            {/* yr2 historical */}
                            {renderHistCell(h2, 12, outlier2)}
                            {/* forecast */}
                            <TableCell className="px-3 py-2 text-center font-mono text-sm font-bold text-primary tabular-nums whitespace-nowrap align-middle">
                              {fc.forecastVol.toLocaleString()}
                            </TableCell>
                            {/* actual */}
                            <TableCell className="px-2 py-1.5 text-center align-middle">
                              {fc.isCompleted ? (
                                <div className="flex items-center justify-center gap-1.5">
                                  <Input
                                    type="number" min={0}
                                    className="h-7 w-28 text-center text-xs font-mono font-bold tabular-nums"
                                    placeholder="Enter actual"
                                    defaultValue={fc.actualVol ?? ""}
                                    onBlur={(e) => {
                                      const val = e.target.value.trim();
                                      if (val !== "" && !isNaN(Number(val)) && Number(val) >= 0) {
                                        void handleSaveActual(fc.index, detailChannel, Number(val));
                                      }
                                    }}
                                    onKeyDown={(e) => {
                                      if (e.key === "Enter") {
                        ...
                        <Table className="table-fixed min-w-[820px]">
                          ...
                          <TableBody>
                            {MONTH_NAMES.map((monthName, monthIdx) => {
                              ...
                              const yr2Forecast = forecastVolumesByChannelYear2[detailChannel][monthIdx];
                              return (
                ...
                {([
                  {
                    ...
                    prompt: `Analyze my demand forecast and tell me:\n1. Looking at historical demand, actual volume, variance, re-cut forecast, and projected future volume — does the forecast look reasonable, or are there months that appear over- or under-estimated?\n2. What does the shape of the demand forecast (trend direction, seasonality, peak and trough timing) suggest about future volume?\n3. What is the single biggest volume uncertainty I should flag to leadership?\nBe specific — use the actual months and volume numbers from my plan.`,
                  },
                  {
                  ...
                  },
                  {
                    ...
                    prompt: `Using my demand scenario data, answer:\n1. What is the total volume delta between the lowest and highest scenario at their peak month — name the specific month and number.\n2. Which demand assumption or historical input appears to drive the most variance between scenarios?\n3. If you had to bet on one scenario being closest to actual demand 12 months from now, which one and why?\nDon't be vague — use the actual scenario names, months, and volume numbers from my plan.`,
                  },
            ...
            <div id="section-assumptions" className="order-2 xl:order-1 xl:col-start-2 xl:row-start-1 xl:sticky xl:top-[180px]">
              <Card className="border border-border/80 shadow-xl overflow-hidden">
                <CardHeader className="border-b border-border/50 bg-[#171717] text-white py-4">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm font-semibold flex items-center gap-2 uppercase tracking-[-0.32px]"><Settings2 className="size-4 text-[#0a72ef]" />Demand Assumptions</CardTitle>
                ...
                <CardContent className="p-0 bg-white divide-y divide-[#ebebeb]">
                  ...
                  <div>
                    <button type="button" className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-[#fafafa] transition-colors" onClick={() => setSbGrowth(v => !v)}>
                      <span className="text-xs font-black uppercase tracking-widest text-[#171717]">Growth Rate</span>
                      <div className="flex items-center gap-2">
                        <Badge className={assumptions.growthRate >= 0 ? "bg-emerald-500 font-black tracking-tight text-xs" : "bg-rose-500 font-black tracking-tight text-xs"}>
                          {assumptions.growthRate >= 0 ? "+" : ""}{assumptions.growthRate}%
                        </Badge>
                        {sbGrowth ? <ChevronUp className="size-3.5 text-[#4d4d4d]" /> : <ChevronDown className="size-3.5 text-[#4d4d4d]" />}
                      </div>
                    </button>
                    {sbGrowth && (
                      <div className="px-4 pb-4 space-y-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <p className="text-[11px] text-[#4d4d4d]">Applied as a post-forecast volume multiplier. Negative values model decline.</p>
                        </div>
                  ...
                  <div className="px-4 py-4">
                    <Button className="w-full h-10 font-black uppercase tracking-widest text-xs shadow-lg shadow-primary/20" onClick={() => toast.info("Demand forecast recalculated", { duration: 1500 })}><LayoutDashboard className="size-4 mr-2" />Recalculate</Button>
                  </div>
