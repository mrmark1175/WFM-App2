...
import {
  ...
  Phone, Mail, MessageSquare, Clock, Users, SlidersHorizontal,
} from "lucide-react";
...
function makeDefaultChannelSchedule(weekdaysEnabled = true): ChannelSchedule {
  return {
    monday:    makeDefaultDay(weekdaysEnabled),
    tuesday:   makeDefaultDay(weekdaysEnabled),
    wednesday: makeDefaultDay(weekdaysEnabled),
    thursday:  makeDefaultDay(weekdaysEnabled),
    friday:    makeDefaultDay(weekdaysEnabled),
    saturday:  makeDefaultDay(false),
    sunday:    makeDefaultDay(false),
  };
}

const DEFAULT_HOURS: HoursOfOperation = {
  voice: makeDefaultChannelSchedule(true),
  email: makeDefaultChannelSchedule(true),
  chat:  makeDefaultChannelSchedule(true),
};

function mergeHours(saved: Partial<HoursOfOperation> | null | undefined): HoursOfOperation {
  const result = { ...DEFAULT_HOURS };
  for (const ch of (["voice", "email", "chat"] as ChannelKey[])) {
    if (saved?.[ch]) {
      result[ch] = { ...makeDefaultChannelSchedule(true), ...saved[ch] };
    }
  }
  return result;
}
...
function applyDefaults(raw: Partial<LobSettings> & { lob_id: number; lob_name: string }): LobSettings {
  return {
    ...
    voice_shrinkage:      raw.voice_shrinkage        ?? 25,
    voice_max_occupancy:  raw.voice_max_occupancy   ?? 85,
    ...
    chat_shrinkage:       raw.chat_shrinkage         ?? 25,
    ...
    email_shrinkage:      raw.email_shrinkage        ?? 25,
...
export function LOBSettings() {
  ...
  // ── Update helper ──────────────────────────────────────────────────────────
  const update = useCallback(<K extends keyof LobSettings>(
  ...
  const updateHours = useCallback((
    lobId: number, channel: ChannelKey, day: DayKey, field: keyof DaySchedule, value: boolean | string
  ) => {
    setSettings((prev) => {
      const next = prev.map((s) => {
        if (s.lob_id !== lobId) return s;
  ...
  const toggleExpanded = (lobId: number) => {
    setExpanded((prev) => {
            ...
            {settings.map((s) => {
              ...
              return (
                <tr
                  ...
                  onClick={() => toggleExpanded(s.lob_id)}
                >
                  ...
                  <td className="py-2 pl-2 pr-4 text-right font-mono text-xs">
                    {voiceDays}d · {voiceHrs}h/day
                  </td>
                </tr>
              );
            })}
  ...
  // ── Channel staffing params section ───────────────────────────────────────
  const ChannelStaffingSection = ({ s, channel }: { s: LobSettings; channel: ChannelKey }) => {
    const meta = CHANNEL_META[channel];
    return (
      <div className={`rounded-lg border border-border/60 p-4 ${meta.bgClass} space-y-4`}>
        <p className={`text-xs font-black uppercase tracking-widest ${meta.colorClass} flex items-center gap-1.5`}>
          <meta.Icon className="size-3.5" />{meta.label} Channel
        </p>
        {channel === "voice" && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            <NumericField label="AHT" value={s.voice_aht} unit="s" min={1} max={9999}
              onChange={(v) => update(s.lob_id, "voice_aht", v)}
            ...
            <NumericField label="SLA Target" value={s.voice_sla_target} unit="%" min={1} max={100}
              onChange={(v) => update(s.lob_id, "voice_sla_target", v)}
              tooltip="% of calls to be answered within SLA seconds" />
            <NumericField label="SLA Seconds" value={s.voice_sla_seconds} unit="s" min={1} max={3600}
              onChange={(v) => update(s.lob_id, "voice_sla_seconds", v)}
              tooltip="Target answer speed (ASA) in seconds for voice SLA" />
            <NumericField label="Max Occupancy" value={s.voice_max_occupancy} unit="%" min={1} max={100}
              onChange={(v) => update(s.lob_id, "voice_max_occupancy", v)}
              tooltip="Maximum allowable agent occupancy for voice (Erlang C output ceiling)" />
            <NumericField label="Shrinkage" value={s.voice_shrinkage} unit="%" min={0} max={99}
              onChange={(v) => update(s.lob_id, "voice_shrinkage", v)}
              tooltip="% of scheduled time agents are unavailable (off-phone activities)" />
          </div>
        )}
        {channel === "chat" && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            <NumericField label="AHT" value={s.chat_aht} unit="s" min={1} max={9999}
              onChange={(v) => update(s.lob_id, "chat_aht", v)}
            ...
            <NumericField label="SLA Target" value={s.chat_sla_target} unit="%" min={1} max={100}
              onChange={(v) => update(s.lob_id, "chat_sla_target", v)}
              tooltip="% of chats to be answered within SLA seconds" />
            <NumericField label="SLA Seconds" value={s.chat_sla_seconds} unit="s" min={1} max={3600}
              onChange={(v) => update(s.lob_id, "chat_sla_seconds", v)}
              tooltip="Target answer speed in seconds for chat SLA" />
            <NumericField label="Concurrency" value={s.chat_concurrency} unit="chats" min={1} max={10} step={0.5}
              onChange={(v) => update(s.lob_id, "chat_concurrency", v)}
              tooltip="Simultaneous chats handled per agent. Reduces effective AHT for Erlang C calculations." />
            <NumericField label="Shrinkage" value={s.chat_shrinkage} unit="%" min={0} max={99}
              onChange={(v) => update(s.lob_id, "chat_shrinkage", v)}
              tooltip="% of scheduled time chat agents are unavailable" />
          </div>
        )}
        {channel === "email" && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            <NumericField label="AHT" value={s.email_aht} unit="s" min={1} max={99999}
              onChange={(v) => update(s.lob_id, "email_aht", v)}
            ...
            <NumericField label="SLA Target" value={s.email_sla_target} unit="%" min={1} max={100}
              onChange={(v) => update(s.lob_id, "email_sla_target", v)}
              tooltip="% of emails to be handled within SLA seconds" />
            <NumericField label="SLA Seconds" value={s.email_sla_seconds} unit="s" min={60} max={259200}
              onChange={(v) => update(s.lob_id, "email_sla_seconds", v)}
              tooltip="Target handling time for email in seconds (e.g. 14400 = 4 hours)" />
            <NumericField label="Utilisation" value={s.email_occupancy} unit="%" min={1} max={100}
              onChange={(v) => update(s.lob_id, "email_occupancy", v)}
              tooltip="Target agent utilisation % for email (async backlog model — this is an INPUT, unlike voice occupancy which is an output)" />
            <NumericField label="Shrinkage" value={s.email_shrinkage} unit="%" min={0} max={99}
              onChange={(v) => update(s.lob_id, "email_shrinkage", v)}
              tooltip="% of scheduled time email agents are unavailable" />
          </div>
        )}
      </div>
    );
  ...
  const HoursTable = ({ s }: { s: LobSettings }) => {
    const enabledChannels = (["voice", "email", "chat"] as ChannelKey[]).filter(
      (c) => s.channels_enabled[c]
    );
        ...
        <table className="w-full text-sm min-w-[640px]">
          <thead>
            ...
            <tr className="border-b border-border/50 bg-muted/20">
              <th className="py-1.5 pl-4 pr-3"></th>
              {enabledChannels.map((ch) => (
                  ...
                  return (
                    <React.Fragment key={ch}>
                      <td className="py-1.5 px-1 border-l border-border/30">
                        <Input
                          type="time" value={sched.open}
                          disabled={!sched.enabled}
                          onChange={(e) => updateHours(s.lob_id, ch, day, "open", e.target.value)}
                          className="h-7 text-xs font-mono px-1.5 disabled:opacity-40 min-w-[80px]"
                        />
                      </td>
                      <td className="py-1.5 px-1">
                        <Input
                          type="time" value={sched.close}
                          disabled={!sched.enabled}
                          onChange={(e) => updateHours(s.lob_id, ch, day, "close", e.target.value)}
                          className="h-7 text-xs font-mono px-1.5 disabled:opacity-40 min-w-[80px]"
                        />
                      </td>
                      <td className="py-1.5 px-1 text-center">
                        <Switch
                          ...
                          onCheckedChange={(v) => updateHours(s.lob_id, ch, day, "enabled", v)}
                          className="scale-75"
                        />
                      </td>
                    </React.Fragment>
  ...
  // ── LOB accordion card ─────────────────────────────────────────────────────
  const LobCard = ({ s }: { s: LobSettings }) => {
    const isOpen    = expanded.has(s.lob_id);
    const isSaving  = saving.has(s.lob_id);
    const hasError  = saveError.has(s.lob_id);
              ...
              <Tabs defaultValue="staffing">
                ...
                <TabsContent value="staffing" className="space-y-6 mt-0">
                          ...
                          <label key={ch} className={`flex items-center gap-2.5 rounded-lg border border-border/60 px-4 py-3 cursor-pointer transition-colors ${s.channels_enabled[ch] ? bgClass : "bg-muted/20 opacity-60"}`}>
                            <Checkbox
                              ...
                              onCheckedChange={(checked) =>
                                update(s.lob_id, "channels_enabled", { ...s.channels_enabled, [ch]: !!checked })
                              }
                            />
                  ...
                  <div className="space-y-3">
                    ...
                    <RadioGroup
                      value={s.pooling_mode}
                      onValueChange={(v) => update(s.lob_id, "pooling_mode", v as PoolingMode)}
                  ...
                  {/* Per-channel staffing params */}
                  <div className="space-y-3">
                    <p className="text-xs font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1.5">
                      <Users className="size-3.5" />Staffing Parameters
                    </p>
                    {(["voice", "email", "chat"] as ChannelKey[])
                      .filter((c) => s.channels_enabled[c])
                      .map((ch) => <ChannelStaffingSection key={ch} s={s} channel={ch} />)
                    }
                    {(["voice", "email", "chat"] as ChannelKey[]).every((c) => !s.channels_enabled[c]) && (
                ...
                <TabsContent value="hours" className="mt-0 space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Set operating hours per channel per day. Disabled days are counted as closed. Times feed into Demand Planner operating hours/days defaults.
                  </p>
                  <HoursTable s={s} />
                </TabsContent>
              </Tabs>
    ...
    return (
      <PageLayout title="LOB Settings">
        <div className="flex items-center justify-center py-24 gap-2 text-muted-foreground">
          ...
          <span>Loading LOB settings…</span>
        </div>
      </PageLayout>
    );
  }
  ...
  return (
    <PageLayout title="LOB Settings">
      <p className="text-sm text-muted-foreground mb-8 max-w-3xl">
        Define channels, staffing parameters, and operating hours for each Line of Business.
        These settings are the default source of truth for Demand Planning, Intraday Forecast, and Capacity Planning.
      ...
      <div className="space-y-4">
        {settings.map((s) => <LobCard key={s.lob_id} s={s} />)}
      </div>
    </PageLayout>
  );
}
