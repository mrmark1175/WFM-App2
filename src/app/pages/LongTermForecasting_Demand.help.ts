...
export const demandForecastHelpSections: DemandHelpSection[] = [
  ...
  },
  {
    ...
    points: [
      ...
      "Shared pools use voice as the priority base. Any idle voice capacity is applied to chat first and then email before extra staffing is added.",
      "Voice base staff comes from Erlang C, chat workload is reduced by concurrency, and email consumes only the idle time that remains after chat.",
  ...
  },
  {
    ...
    points: [
      "Voice uses the page forecast volume, the configured voice AHT, and Erlang C against the voice SLA target.",
      "Email uses channel forecast volume and a deferred backlog model that clears the SLA-target share of daily workload inside the email response window.",
      "Chat uses channel forecast volume and modified Erlang C with concurrency of 2, which reduces effective handling time per staffed seat.",
  ...
  },
  {
    ...
    points: [
      "Monthly forecast workload is converted into average concurrent demand intensity during the configured open hours.",
      "Voice uses Erlang C to find the minimum staffed seats that achieve the voice SLA target, while chat uses the same approach with effective AHT divided by the configured chat concurrency.",
      "Email uses a deferred-work model where agents must both absorb average workload and clear the SLA-target share of daily backlog within the response window.",
      "Required occupancy is not keyed in manually. It is derived after staffing as Intensity divided by Agents for each channel.",
      "Gross FTE converts staffed seats into headcount by covering monthly open hours, then applying shrinkage and safety margin.",
